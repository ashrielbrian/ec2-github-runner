...
Alternatively, you can use a vanilla EC2 AMI and setup the dependencies setup via `pre-runner-script` in the workflow YAML file:
```yaml
...
| `pre-runner-script`                                                                                                                                                              | Optional. Used only with the `start` mode. | Specifies bash commands to run before the runner starts.  It's useful for installing dependencies with apt-get, yum, dnf, etc. For example:<pre>          - name: Start EC2 runner<br>            with:<br>              mode: start<br>              ec2-image-id: ami-07bdb09c49774f92e<br>              pre-runner-script: \|<br>                 sudo yum update -y && \ <br>                 sudo yum install docker git libicu -y<br></pre> |
...
| `block-device-mapping`                                                                                                                                                              | Optional. Used only with the `start` mode. | JSON string specifying the [BlockDeviceMapping](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ec2-blockdev-mapping.html).  For example:<pre> block-device-mapper: \|<br>   [<br>   {"DeviceName" : "/dev/sda1", "Ebs" : { "VolumeType": "gp2", "VolumeSize": 34 }},<br>   {"DeviceName" : "/dev/sdb", "VirtualName": "ephemeral0" }<br>   ] </pre> |
`save-console-file`                                                                                                                                                              | Optional. Used in `start` or `stop` mode, but most useful in `stop`. | Save the most recent output of the instance's serial console to a file named `$save-console-file` on the runner.  This file can then be uploaded with [Artifacts](https://docs.github.com/en/actions/using-workflows/storing-workflow-data-as-artifacts).  The console output can be useful for debugging kernel panics. For example:<pre>      - name: Stop EC2 runner<br>        uses: machulav/ec2-github-runner@v2<br>        with:<br>          mode: stop<br>          github-token: ${{ secrets.GH_PERSONAL_ACCESS_TOKEN }}<br>          label: ${{ needs.start-runner.outputs.label }}<br>          ec2-instance-id: ${{ needs.start-runner.outputs.ec2-instance-id }}<br>          save-console-file: console.txt<br>      \- name: Upload console logs<br>        uses: actions/upload-artifact@v3<br>        with:<br>          name: archive_of_output_files<br>          path: console.txt<br>
</pre>
<br><br> |
